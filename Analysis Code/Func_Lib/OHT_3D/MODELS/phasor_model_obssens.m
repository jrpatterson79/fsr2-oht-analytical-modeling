...
%TODO: Test Sy support
%12th input, if supplied, is treated as Sy matrix. NOT YET supported
Sy = [];
if nargin > (num_reqin)
    Sy = varargin{1};
end
%13th input, if supplied, is leakances to the boundaries. NOT YET tested
%TODO: Verify Lb support
Lb = [];
if nargin > (num_reqin + 1)
    Lb = varargin{2};
end

%15th input, if supplied, is function used to invert key matrices. Matlab \
%operator is the default.
%TODO: Try using another matrix inversion scheme (e.g., a bicgstab wrapper
%that accomodates complex matrices and multiple right-hand sides)
matinv_func = 'mldivide';
if nargin > (num_reqin+2)
    if ~isempty(varargin{3})
        matinv_func = varargin{3};
    end
end

[A_steady,A_omegamod,b_bccoeff] = phasor_model_form(K,Ss,xb,yb,zb,bdry_types,Sy,Lb);
...
A_complex = (A_steady + A_omegamod*1i*omega);
...
Phi = feval(matinv_func,A_complex,b_allstims);
...
if nargout > 2
    ...
    H{3} = zeros(num_obs,num_y*num_x);

    %Solve for the adjoint variable first
    lambda = feval(matinv_func,A_complex.',obsweight_list);    
    ...
    for t = 1:1:num_stims
        ...
        %TODO Seems like this is wrong - doesn't actually follow the same
        %structure as model_form code, since the transpose thing isn't done
        %(but maybe I'm thinking about this wrong). Check into whether some
        %are being cutoff.
    ...
    for t = 1:1:num_stims
        Phi_currstim = Phi(:,t);
        dbdSs_minus_dAdSsPhi_diags = -omega.*1i.*reshape(dxdydz,num_cells,1)...
            .*Phi_currstim;

        dbdSs_minus_dAdSsPhi{t} = spalloc(num_cells,num_cells,num_cells);
        dbdSs_minus_dAdSsPhi{t} = spdiags(dbdSs_minus_dAdSsPhi_diags,[0],...
    ...
    if ~isempty(Sy)
        ...
        for t = 1:1:num_stims
            ...
            dAdSy = zeros(num_cells,2);
            dAdSy(:,1) = reshape(dAdSy_center,num_cells,1);
            dAdSy(:,2) = reshape(dAdSy_bottom,num_cells,1);
            Phi_currstim = Phi(:,t);
            Phic = reshape(Phi_currstim,num_y,num_x,num_z);
            Phib = zeros(num_y,num_x,num_z);
            Phib(:,:,2:end) = Phic(:,:,1:(end-1)); Phib = reshape(Phib,num_cells,1);
            Phic = reshape(Phic,num_cells,1);
            dbdSy_minus_dAdSyPhi_diags(:,1) = -dAdSy(:,1).*Phic -dAdSy(:,2).*Phib;
            dbdSy_minus_dAdSyPhi{t} = spalloc(num_cells,num_cells,num_cells);
            dbdSy_minus_dAdSyPhi{t} = spdiags(dbdSy_minus_dAdSyPhi_diags,[0],...
        ...
        for i = 1:1:num_obs
            ...
            %TODO: Seems to be constant error when compared against FD
            %computation... rounding errors?
            Sy_int_sens = reshape(full(lambda(:,obs_type).'*dbdSy_minus_dAdSyPhi{test_type}),num_y,num_x,num_z);
            Sy_int_top = reshape(Sy_int_sens(:,:,end),num_y*num_x,1);
            H{3}(i,:) = Sy_int_top;
        end
    end
    varargout{2} = H;
