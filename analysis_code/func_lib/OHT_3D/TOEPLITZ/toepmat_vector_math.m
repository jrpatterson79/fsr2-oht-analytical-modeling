...
%covariance matrices for a regular, equispaced grid. The function currently
%performs 4 different types of computations:
%  1) Toeplitz matrix - vector product
%  2) Toeplitz matrix inverse - vector product (beta / may not apply in all
%  cases)
...
% ***NOTE: Only matrix-vector products and conditional realizations are
% working / tested.
%
...
%      representing
%      4) Realizations: result as a (n x 2) output containing 2 conditional
...
% -Tested for multiplication, realizations in multiple dimensions. Seems
% to give correct results.
% -Eigenvalue function and division (especially) have not been tested and
% may not be functional yet.
%
% Code by Michael Cardiff
% 8/2009
% Based on reading of W. Nowak Thesis & papers
...
elseif toep_level == 3
    ...
    elseif strcmp(operation,'r')
        ...
        e2 = randn(cdims);
        ec = e1 + 1i*e2;
        fft_product = fftn(circmat_rowmat.*n).^.5.*ec; %Appears that a multiplication correction factor is needed here, unlike presented in Dietrich, Nowak (who do division).
        result_padded = ifftn(fft_product);
        result = result_padded(1:disc_vec(1),1:disc_vec(2),1:disc_vec(3));
        result = reshape(result,numel(result),1);
        result = [real(result) imag(result)];
